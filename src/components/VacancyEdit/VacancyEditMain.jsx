...
const VacancyEditMain = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                setLocations([data.area]);
                setFormats([WORK_FORMAT_LABELS[data.work_format] || "Любой"]);
    ...
    /* salary helpers */
    const handleFrom = (v) => setSalaryFrom(v);
    const handleTo = (v) => setSalaryTo(v);

    /* enum маппинг */
    const mapExp = () => (experience === "moreThan6" ? "between3and6" : experience);
    ...
    const buildPayload = () => ({
        ...
        area: locations.find(l => l !== "Любой город") || "Любой город",
        ...
        work_format: LABELS_TO_FORMAT[formats.find(f => f !== "Любой")] || null,
        work_time: workHours,
        work_schedule: workSchedule,
        required_experience: mapExp(),
    });

    /* запросы */
    const patchVacancy = async (data) => {
        try {
            await axiosInstance.patch(`manager/vacancies/${id}`, data);
    ...
    const toggleItem = (v, list, setList) =>
            ...
            <div className="vacancy-create__container">
                ...
                {step === 2 && (
                    <>
                        ...
                        <div className="buttons">
                            <button className="btn btn-back" onClick={() => setStep(1)}>Назад</button>
                        </div>
                    </>
                )}
            </div>
